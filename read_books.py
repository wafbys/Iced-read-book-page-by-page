...
PARTIAL_SUMMARY_PROMPT = """你在做全书总结的**中间稿**（高质量优先）。
...
FINAL_SUMMARY_PROMPT = """你是技术书导读作者，目标是让读者能对照 PDF 精读。
目标书为**中文、英文或中英混排**。根据输入写成**完整 Markdown 终稿**。

## 必须结构（顺序固定）
1. `## 导读`：2–4 句：问题意识、主线贡献、阅读地图（可含关键页码）
2. `## 分题详述`：### 分主题，名贴近原书；按逻辑递进
3. `## 主题索引`：`- 词条（English 若原文有）：12, 42–43` 形式；禁止按页倾倒全部原文
