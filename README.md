...
|------|------|
| **原书 PDF** | 完整内容；精读以它为准 |
| **`书名.md`** | 精读地图：怎么读 + 带阅读器页码的逻辑单位 + 速查 |
| **`书名_knowledge.json`** | 页级知识点清单 + 进度/策略；比地图细，供生成用 |

**推荐用法**：把 md 打出来，按单位上的页码打开 PDF，用笔在纸上划。  

**不必指望**：

- 地图穷尽 knowledge 里每一条（一个单位是一块说得清的原文，不是页上全部要点）  
- md ≈ 全书论点复述（细节以 PDF 为准，空白留给你写）  
